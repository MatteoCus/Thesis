...
Questo capitolo introduce gli strumenti utilizzati nel corso del tirocinio, definisce una visione concreta delle attività di sviluppo del progetto di \textit{stage} e fornisce prova dei risultati raggiunti
...
detti "requisiti utente", e la definizione di "ciò che deve svolgere il prodotto" per soddisfare tali bisogni, i "requisiti del prodotto". \\
...
\subsubsection*{Lista dei principali casi d'uso}
    \textbf{UCF-2: Visualizzazione delle fasi}
      ...
      nè tantomeno è generalizzabile, ovvero non è possibile individuare ereditarietà con altri casi d'uso.
      \newpage
...
Con "requisito" si intende una specifica di ciò che un sistema \textit{software} deve fare o possedere al fine di soddisfare un determinato obiettivo o risolvere un particolare problema; i requisiti sono le basi per la \textbf{progettazione} e la \textbf{verifica} di un \textit{software}. \\
...
\begin{longtable}{>{\centering\arraybackslash}m{0.13\textwidth}>{\centering\arraybackslash}m{0.66\textwidth}>{\centering\arraybackslash}m{0.20\textwidth}>{\centering\arraybackslash}m{0.15\textwidth}}
    ...
    \hline
    RP-2 & L’interfaccia utente deve adattarsi a \textit{smartphone} & Facoltativo \\
    \hline
...
\end{itemize}
Di seguito riporto l'architettura delle classi relative ai casi d'uso fondamentali, indicati nella sezione \hyperref[subsubsec:casi-d'uso]{§3.3.1}.
...
Di seguito riporto il contenuto dei file \textit{TypeScript} relativi alle classi derivate dai casi d'uso descritti nella sezione \hyperref[subsubsec:casi-d'uso]{§3.3.1}.
...
Durante il periodo di \textit{stage}, ho eseguito attività di \textbf{analisi statica} manualmente (senza quindi avvalermi di strumenti \textit{software}) per quanto riguarda la documentazione di progetto ed il codice sorgente, data l'assenza di direttive in relazione allo stile di scrittura: 
...
Le attività di \textbf{validazione} servono per accertare che il prodotto finito sia conforme alle aspettative di chi ha la visione dei bisogni da soddisfare con il prodotto (proponente). \\
Ho effettuato il collaudo durante l'ultima settimana di attività in presenza del committente, ovvero colui (persona fisica, azienda o ente) che ha interesse diretto nel risultato finale del progetto: nel mio caso, le figure del proponente e del committente sono rappresentate dal \textit{tutor} aziendale. \\
...
% In questa sezione descriverò i prodotti di progetto dal punto di vista della qualità (qui vi sarà particolare enfasi sul prodotto \textit{software}) e della quantità.
...
Con "\textit{code coverage}" si fa riferimento a una serie di metriche utilizzate nel campo dello sviluppo \textit{software} per valutare quanto del codice sorgente di un'applicazione è stato eseguito durante l'esecuzione di un insieme di \textit{test}; non garantisce necessariamente la qualità dei \textit{test} eseguiti 
...
\begin{figure}[H]
  ...
  \caption{Schermata principale - tema scuro}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{user-manual/dashboard-light.png}
  \caption{Schermata principale - tema chiaro}
\end{figure}
